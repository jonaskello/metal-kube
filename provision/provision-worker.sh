...
readarray RESULT < <(ssh $PROVISION_MASTER_SSH "dpkg-query --showformat='\${Version}' --show docker-ce && echo && dpkg-query --showformat='\${Version}' --show kubeadm && echo && kubeadm token create --print-join-command --ttl=1h0m0s")
